...
| **`aca sandbox exec` timing** | Blocks until the command completes. `npm install` takes 2–5 minutes (egress proxy), `git clone` 30s–2min, `npm run build` 1–3min. Do NOT assume the command is stuck. |
...
| Issue | Solution |
|-------|----------|
| `aca sandbox fs put` creates files but not directories | `mkdir -p /path/to/dir` first via `aca sandbox exec` |
